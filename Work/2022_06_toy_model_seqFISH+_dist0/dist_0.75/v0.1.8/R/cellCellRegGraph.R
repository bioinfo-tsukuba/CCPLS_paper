...
#' 
#' @param all_cell_type_signif_coefMN List of coefficient in each cell type estimated as significant
#' @param cell_type_list Cell type list
#' @param dev_opt Option for development mode
#' 
#' @importFrom dplyr %>%
#' @importFrom multigraph bmgraph
#' 
cellCellRegGraph <- function(res.sel.var, output_dir){
  ...
  for (cell_type_ind in 1:cell_type_num){
    ...
    if (!is.null(res.sel.var$all_zero_flag_list[[cell_type_ind]])){
      if (!res.sel.var$all_zero_flag_list[[cell_type_ind]]){
        ...
        png(paste0(my_working_dir, output_dir, "/", cell_type_ind, "_bipartite_graph.png"))    
        plot(g,
             vertex.color = "white",
        ...
        pdf(paste0(my_working_dir, output_dir, "/", cell_type_ind, "_bipartite_graph.pdf"))    
        plot(g,
             vertex.color = "white",
             vertex.frame.color = "black",
             vertex.label.color = "black",
        ...
        sink(paste0(my_working_dir, output_dir, "/", cell_type_ind, "_bipartite_graph.txt"))
