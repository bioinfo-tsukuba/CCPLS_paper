Sim_exp_mat_v15.4_internal <- function(exp_mat, coord_mat, annot_mat,
  ...
  if (output_opt){
    ...
    # Prepare raw data
    exp_mat_sim_z <- exp_mat_raw_sep_list_sim[[1]]
    
    tot_counts_vec <- rowSums(seur_obj@assays$RNA@counts)
    sim_col <- colnames(exp_mat_sim_z)
    tot_counts_vec_2 <- tot_counts_vec[sim_col]
    
    exp_mat_sim_z_2 <- exp(exp_mat_sim_z)
    exp_mat_sim_z_3 <- exp_mat_sim_z_2 - 1
    
    exp_mat_sim_z_4 <- exp_mat_sim_z_3
    exp_mat_sim_z_4[exp_mat_sim_z_3 < 0] <- 0
    
    # sum(colnames(exp_mat_sim_z_3) == names(tot_counts_vec_2)) == 2000
    exp_mat_sim_z_5 <- exp_mat_sim_z_4
    for (col_id in 1:ncol(exp_mat_sim_z_4)){
      exp_mat_sim_z_5[,col_id] <- exp_mat_sim_z_4[,col_id] * tot_counts_vec_2[col_id]
    }
    
    exp_mat_sim_z_6 <- exp_mat_sim_z_5
    for (cell_id in 1:nrow(exp_mat_sim_z_5)){
      scaling_factor_each <- sum(exp_mat_l5[cell_id, HVG_list_l5]) / sum(exp_mat_sim_z_6[cell_id, HVG_list_l5])
      exp_mat_sim_z_6[cell_id,] <- exp_mat_sim_z_5[cell_id,] * scaling_factor_each
    }
    
    exp_mat_sim_z_7 <- exp_mat
    exp_mat_sim_z_7[rownames(exp_mat_l5), HVG_list_l5] <- exp_mat_sim_z_6
    # cor.test(rowSums(exp_mat_sim_z_7), rowSums(exp_mat_l5[,HVG_list_l5]))

    write.table(exp_mat_sim_z_7,
                paste0("~/CCPLS_paper/Work/2021_11_toy_model_seqFISH+/simulated_data_5/exp_mat_w_",
                       weight_const, "_alpha_", e_g_i_const, "_all.tsv"),
                sep = "\t",
                row.names = FALSE)
