...
## Prepare
rm(list = ls())
setwd("~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt")
```


```{r}
# Reading files
my_working_dir <- "~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt"
...
write.table(coord_mat_seqFISHplus[annot_mat_seqFISHplus[,3] == "L5 eNeuron",],
            "~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt/simulated_data_5/coord_mat.tsv",
            sep = "\t",
            row.names = FALSE)
...
# save annot_mat
write.table(annot_mat_toy, file = "~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt/simulated_data_5/annot_mat_toy.tsv",
sep = "\t",
row.names = FALSE)

# save coord_mat
write.table(coord_mat_seqFISHplus, file = "~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt/simulated_data_5/coord_mat_toy.tsv",
sep = "\t",
row.names = FALSE)
...
output_exp_name <- paste0("~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt/seqFISH+_real_data/exp_mat_all.tsv") 
output_coord_name <- paste0("~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt/seqFISH+_real_data/coord_mat_all.tsv") 
output_annot_name <- paste0("~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt/seqFISH+_real_data/annot_mat_all.tsv") 
...
for (ct_ind in 1:length(ct_list)){
  ...
  output_exp_name <- paste0("~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt/seqFISH+_real_data/exp_mat_", ct_ind, ".tsv") 
  
  output_coord_name <- paste0("~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt/seqFISH+_real_data/coord_mat_", ct_ind, ".tsv") 
  
  output_annot_name <- paste0("~/CCPLS_paper/Work/2022_07_toy_model_seqFISH+_two_filt/seqFISH+_real_data/annot_mat_", ct_ind, ".tsv") 
...
# Save
save.image("~/CCPLS_Paper/Work/2022_07_toy_model_seqFISH+_two_filt/results_collect/res.list.RData")
...
```{r}
# Examine filtered genes
fo_coef_vec_fet_neig_B <- readRDS(paste0(my_working_dir, "/results_v0.1.8_w_1_alpha_1/",
                                         "fo_coef_vec_fet_neig_B.rds")) # filtered in step 2

sig_coef_mat_raw <- readRDS(paste0(my_working_dir,
                                   "/results_v0.1.8_w_1_alpha_1/sig_coef_mat_raw.rds"))
sig_coef_mat_step_1 <- readRDS(paste0(my_working_dir,
                                   "/results_v0.1.8_w_1_alpha_1/sig_coef_mat_step_1.rds"))
sig_coef_mat_step_2 <- readRDS(paste0(my_working_dir,
                                   "/results_v0.1.8_w_1_alpha_1/sig_coef_mat_step_2.rds"))

step_1_flag <- sig_coef_mat_step_1[2,] == 0
step_2_flag <- sig_coef_mat_step_1[2,] != 0 & sig_coef_mat_step_2[2,] == 0
rem_flag <- sig_coef_mat_step_2[2,] > 0

df_coef_1 <- cbind(sig_coef_mat_raw[2, step_1_flag], "1. First filtered")
colnames(df_coef_1) <- c("Coefficient", "Type")
df_coef_1 <- as_tibble(df_coef_1)
df_coef_1$Coefficient <- as.double(df_coef_1$Coefficient)

p_hist_1 <- ggplot(as_tibble(df_coef_1), aes(x = Coefficient, fill = Type)) +
              geom_histogram(alpha = 0.8, color = "Green") +
              theme(panel.grid.minor = element_blank()) +
              theme_bw()

df_coef_2 <- cbind(sig_coef_mat_raw[2, step_2_flag], "2. Second filtered")
colnames(df_coef_2) <- c("Coefficient", "Type")
df_coef_2 <- as_tibble(df_coef_2)
df_coef_2$Coefficient <- as.double(df_coef_2$Coefficient)

p_hist_2 <- ggplot(as_tibble(df_coef_2), aes(x = Coefficient, fill = Type)) +
              geom_histogram(alpha = 0.8) +
              theme(panel.grid.minor = element_blank()) +
              theme_bw()

df_coef_3 <- cbind(sig_coef_mat_raw[2, rem_flag], "3. Passed")
colnames(df_coef_3) <- c("Coefficient", "Type")
df_coef_3 <- as_tibble(df_coef_3)
df_coef_3$Coefficient <- as.double(df_coef_3$Coefficient)

p_hist_3 <- ggplot(as_tibble(df_coef_3), aes(x = Coefficient, fill = Type)) +
              geom_histogram(alpha = 0.8) +
              theme(panel.grid.minor = element_blank()) +
              theme_bw()

df_sum <- rbind(df_coef_1, df_coef_2)
df_sum <- rbind(df_sum, df_coef_3)

p_hist <- ggplot(df_sum, aes(x = Coefficient, fill = Type)) +
              geom_histogram() +
              theme(panel.grid.minor = element_blank()) +
              theme_bw() +
              facet_wrap(Type ~ ., ncol = 1)

ggsave(paste0(my_working_dir,
              "/results_v0.1.8_w_1_alpha_1/histogram.png"),
       p_hist, width = 12, height = 8)


ans_1_pre <- read.table(paste0(my_working_dir, "/answer_gene/cluster_1.txt"))
ans_2_pre <- read.table(paste0(my_working_dir, "/answer_gene/cluster_2.txt"))
ans_3_pre <- read.table(paste0(my_working_dir, "/answer_gene/cluster_3.txt"))
ans_4_pre <- read.table(paste0(my_working_dir, "/answer_gene/cluster_4.txt"))
...
fo_1 <- names(fo_coef_vec_fet_neig_B)[names(fo_coef_vec_fet_neig_B) %in% ans_1]
fo_2 <- names(fo_coef_vec_fet_neig_B)[names(fo_coef_vec_fet_neig_B) %in% ans_2]
fo_3 <- names(fo_coef_vec_fet_neig_B)[names(fo_coef_vec_fet_neig_B) %in% ans_3]
fo_4 <- names(fo_coef_vec_fet_neig_B)[names(fo_coef_vec_fet_neig_B) %in% ans_4]

sum(names(step_1_flag)[step_1_flag] %in% ans_2)
sum(names(step_2_flag)[step_2_flag] %in% ans_2)
sum(names(rem_flag)[rem_flag] %in% ans_2)

length(fo_coef_vec_fet_neig_B)
sum(fo_coef_vec_fet_neig_B)
mean(fo_coef_vec_fet_neig_B)
max(fo_coef_vec_fet_neig_B)
min(fo_coef_vec_fet_neig_B)
```
