performExperiment <- function(my_working_dir,
  ...
  # Simulate
  source(paste0(my_working_dir, "/function/Sim_exp_mat_rev_internal.R"))
  res.sim <- Sim_exp_mat_rev_internal(exp_mat, coord_mat, annot_mat,
  ...
  var_sim <- apply(res.sim$exp_mat_raw_sim_k_list[[1]], 2, var)
  var_tot <- apply(res.sim$exp_mat_raw_sep_list[[1]], 2, var)
  var_prop <- var_sim / var_tot
  ...
  #var_tot <- apply(exp_mat_raw_sim_k_sum, 2, var)
  #var_sd <- apply(noise_mat_z, 2, var)
